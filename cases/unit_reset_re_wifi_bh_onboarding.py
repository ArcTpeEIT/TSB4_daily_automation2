#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Unit test: Reset RE, WiFi BH onboarding only.

Assumes environment is already on WiFi BH (no relay switching).

Flow per loop:
  1. GUI: Reset RE
  2. Wait post_reset_wait (default 30s) for RE reset + raspi5 LAN recovery
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Unit test: Reset RE WiFi BH onboarding (no relay switch)"
    )
    add_common_args(parser)
    parser.add_argument("--wifi-init-wait", type=int, default=cfg.CASE9_WIFI_ONBOARDING_INIT_WAIT_TIME)
    parser.add_argument("--max-total-limit", type=int, default=cfg.CASE9_MAX_TOTAL_LIMIT)
    parser.add_argument(
        "--post-reset-wait", type=int, default=30,
        help="Seconds to wait after Reset button press before starting captures (default: 30)",
    )
    parser.add_argument(
...
def apply_args(args):
    ...
    cfg.CASE9_WIFI_ONBOARDING_INIT_WAIT_TIME = args.wifi_init_wait
    cfg.CASE9_MAX_TOTAL_LIMIT = args.max_total_limit
...
if __name__ == "__main__":
    cfg.TEST_CASE_NAME = "unit_Reset RE WiFi BH Onboarding"
    args = parse_args()
    apply_args(args)
    init_log_filenames()
