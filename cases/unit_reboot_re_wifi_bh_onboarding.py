#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Unit test: Reboot RE, WiFi BH onboarding only.

Assumes environment is already on WiFi BH (no relay switching).

Flow per loop:
  1. GUI: Reboot RE
  2. Wait post_reboot_wait (default 20s) for RE reboot + raspi5 LAN recovery
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Unit test: Reboot RE WiFi BH onboarding (no relay switch)"
    )
    add_common_args(parser)
    parser.add_argument("--wifi-init-wait", type=int, default=cfg.CASE8_WIFI_ONBOARDING_INIT_WAIT_TIME)
    parser.add_argument("--max-total-limit", type=int, default=cfg.CASE8_MAX_TOTAL_LIMIT)
    parser.add_argument(
        "--post-reboot-wait", type=int, default=20,
        help="Seconds to wait after Reboot button press before starting captures (default: 20)",
    )
    parser.add_argument(
...
def apply_args(args):
    ...
    cfg.CASE8_WIFI_ONBOARDING_INIT_WAIT_TIME = args.wifi_init_wait
    cfg.CASE8_MAX_TOTAL_LIMIT = args.max_total_limit
...
if __name__ == "__main__":
    cfg.TEST_CASE_NAME = "unit_Reboot RE WiFi BH Onboarding"
    args = parse_args()
    apply_args(args)
    init_log_filenames()
