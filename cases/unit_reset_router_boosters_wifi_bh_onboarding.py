#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Unit test: Reset Router+Boosters, WiFi BH onboarding only.

Assumes environment is already on WiFi BH (no relay switching).

Flow per loop:
  1. GUI: Reset Router+Boosters
  2. Wait post_reset_wait (default 100s) for TSM4 reboot + raspi5 LAN recovery
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Unit test: Reset Router+Boosters WiFi BH onboarding (no relay switch)"
    )
    add_common_args(parser)
    parser.add_argument("--wifi-init-wait", type=int, default=cfg.CASE7_WIFI_ONBOARDING_INIT_WAIT_TIME)
    parser.add_argument("--max-total-limit", type=int, default=cfg.CASE7_MAX_TOTAL_LIMIT)
    parser.add_argument(
        "--post-reset-wait", type=int, default=100,
        help="Seconds to wait after Reset button press before starting captures (default: 100)",
    )
    parser.add_argument(
...
def apply_args(args):
    ...
    cfg.CASE7_WIFI_ONBOARDING_INIT_WAIT_TIME = args.wifi_init_wait
    cfg.CASE7_MAX_TOTAL_LIMIT = args.max_total_limit
    ...
    if args.wifi_bh_tcpdump:
        cfg.WIFI_BH_TCPDUMP_FILTER      = ""                    # capture all ath1 traffic
        cfg.WIFI_BH_TCPDUMP_MAX_PACKETS = 0                     # no packet limit, stop via kill
        cfg.WIFI_BH_TCPDUMP_REMOTE_PATH = "/tmp/wifi_bh_ath1.pcap"


if __name__ == "__main__":
    cfg.TEST_CASE_NAME = "unit_Reset Router+Boosters WiFi BH Onboarding"
    args = parse_args()
    apply_args(args)
    init_log_filenames()
    ...
    try:
        ok = run_gui_action_wifi_bh_unit_case(
            cfg.XPATH_RESET_ALL,
            "Reset Router+Boosters",
            cfg.CASE7_MAX_TOTAL_LIMIT,
            cfg.RESET_ONBOARDING_THRESHOLD,
            cfg.CASE7_WIFI_ONBOARDING_INIT_WAIT_TIME,
            post_reset_wait=args.post_reset_wait,
