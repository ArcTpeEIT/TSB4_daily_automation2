...
"""
main_wifi_modify_ssid_key_sync_check.py

用途:
    透過 TSM4 GUI 修改 main WiFi SSID / key，並透過 RE serial 檢查設定是否同步。

測試流程:
    1. ETH BH: relay 6 on
    2. GUI 修改第一組隨機 SSID / key
    ...
    10. 若 WiFi BH PASS 且還有下一個 loop，先切回 ETH BH 並等待 LOOP_ETH_RESTORE_WAIT 秒
...
執行:
    python main_wifi_modify_ssid_key_sync_check.py
"""
...
# Random SSID / key
ETH_SSID_PREFIX = "ETHSYNC"
WIFI_SSID_PREFIX = "WIFISYNC"
SSID_RANDOM_LEN = 8
...
XPATH_APPLY = "/html/body/app-root/app-main-base/div/div/main/app-wifi-main/div/div/div/app-wifi-basic/form/div[4]/div/button[2]"

XPATH_SSID = "/html/body/app-root/app-main-base/div/div/main/app-wifi-main/div/div/div/app-wifi-basic/form/div[2]/div[8]/app-label-input/div/div[2]/input"
XPATH_WIFI_PASS = "/html/body/app-root/app-main-base/div/div/main/app-wifi-main/div/div/div/app-wifi-basic/form/div[3]/div[8]/div/div/div/input"
...
TEST_CASE_NAME = "case10_Main_WiFi_Random_SSID_Key_Sync_SpecialChar"
...
def check_re_wifi_sync(expected_ssid, expected_key):
    ...
    if failures:
        log("RE WiFi sync check FAIL")
        for item in failures:
            log(f"  - {item}")
        return False, "; ".join(failures)

    log("RE WiFi sync check PASS")
    return True, "None"
...
def change_wifi_settings(driver, wait, ssid, wifi_password):
    log(f"修改 SSID = {ssid}")
    ssid_input = wait.until(EC.presence_of_element_located((By.XPATH, XPATH_SSID)))
    driver.execute_script("arguments[0].scrollIntoView({block: 'center'});", ssid_input)
    receive_monitor(AFTER_FIELD_SCROLL_WAIT)
    js_set_input_value(driver, ssid_input, ssid)

    log("修改 WiFi Password")
    wifi_pass_input = wait.until(EC.presence_of_element_located((By.XPATH, XPATH_WIFI_PASS)))
...
def apply_settings(driver, wait):
    log("按下 Apply 並等待設定儲存")
    apply_btn = wait.until(EC.presence_of_element_located((By.XPATH, XPATH_APPLY)))
...
def run_one_stage(loop, interface_name, ssid, key):
    log_separator(f"LOOP {loop} - {interface_name} main WiFi modify + sync check")
...
def run_wifi_stage_after_eth_pass(loop, ssid, key):
    ...
    進入本函式後：
        ...
        2. 透過 TSM4 GUI 修改 WiFi BH 用的 SSID/key。
        3. 等 WIFI_AFTER_GUI_APPLY_MONITOR_TIME 秒。
        4. 最後 SSH check RE UCI SSID/key。

    注意：check_re_wifi_sync() 不再執行 WiFi_inf_ChOnOff.sh。
    ...
    log_separator(f"LOOP {loop} - {interface_name} main WiFi modify + sync check")
    log(f"Target SSID={ssid}, key=<hidden>")
    log("WiFi BH 新流程：PASS_COOLDOWN_TIME 後立刻 relay 6 off，再進 GUI 修改 SSID/key")
...
def run_test():
    ...
    log_separator("Main WiFi modify SSID/key sync check start")
    ...
    try:
        ...
        if overall_pass:
            log_separator("Main WiFi modify SSID/key sync check PASS")
        else:
            log_separator("Main WiFi modify SSID/key sync check FAIL")
...
def parse_args():
    parser = argparse.ArgumentParser(description="Main WiFi SSID/key modify and RE sync check")
    ...
    parser.add_argument("--eth-ssid-prefix", default=ETH_SSID_PREFIX, help="ETH BH random SSID prefix")
    parser.add_argument("--wifi-ssid-prefix", default=WIFI_SSID_PREFIX, help="WiFi BH random SSID prefix")
    parser.add_argument("--headless", action="store_true", help="Run Chrome in headless mode")
    parser.add_argument("--no-signout", action="store_true", help="Do not sign out after applying settings")
    parser.add_argument("--loop-eth-restore-wait", type=int, default=LOOP_ETH_RESTORE_WAIT, help="Seconds to wait after WiFi BH PASS and switching back to ETH BH before next loop")
...
def apply_args(args):
    ...
    LOOP_ETH_RESTORE_WAIT = getattr(args, "loop_eth_restore_wait", LOOP_ETH_RESTORE_WAIT)
